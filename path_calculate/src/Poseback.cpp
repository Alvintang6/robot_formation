...
void Poseback::rotation_left(struct robot & rob, float x,float y){
	 x = 1.0029*x-0.0049*x*x-0.009*y+0.0318; y = 0.4173*y+0.0436*x-0.1342*x*x+0.0666;   // using lsqcurvefitting to conpensate camera
...
void Poseback::rotation_right(struct robot & rob, float x,float y){
	 x = 1.108*x-0.1189*x*x-0.0050*y-0.0375;  y = 0.6287*y-0.037*y*y-0.0492*x-0.057*x*x-0.2257;  
	rob.distancex = 0.4226*x+0.9063*y;
	rob.distancey = -0.9063*x+0.4226*y;
	rob.find_right = 1;
}
...
void Poseback::pose_getleftCB(const ar_track_alvar_msgs::AlvarMarkers &msg) {
  ...
  robot1.find_left = 0; robot3.find_left = 0;
...
//check marker from #2 robot (should add bool as flag)
    for(i=0;i<mkr_cnt;i++){
     ...
     pose_solve(3,i,robot3, msg, rotation_left);
...
void Poseback::pose_getrightCB(const ar_track_alvar_msgs::AlvarMarkers &msg) {
         ...
         robot1.find_right = 0; robot3.find_right = 0;
...
//check marker from #3 robot (should add bool as flag)
     for(i=0;i<mkr_cnt;i++){
     
     pose_solve(3,i,robot3, msg, rotation_right);
