...
while( ((ret=vmu_read_all(vmu, &data)) != VMU_ERROR))
{
    ...
    //should add calibration bias to  received angular velocity and pass to to corrected IMU data object
		ang_vel_x = data.gyro[i].x*0.01745329;
  		ang_vel_y = data.gyro[i].y*0.01745329;
  		ang_vel_z = data.gyro[i].z*0.01745329;
	}
	...
	if((gyro_count<calib_offset) && (gyro_count>calib_inital))
	{
		ang_bias_x += ang_vel_x;
		ang_bias_y += ang_vel_y;
		ang_bias_z += ang_vel_z;
	}
	...
	if(gyro_count==calib_offset){
			...
			gyro_inital_x = gyro_temp_x;
		        gyro_inital_y = gyro_temp_y;
			gyro_inital_z = gyro_temp_z;
			ang_bias_x /=calib_simple;	
	  		ang_bias_y /=calib_simple;
			ang_bias_z /=calib_simple;
	...
	if(gyro_count>calib_offset){
		...
		vmu_data.angular_velocity.x = ang_vel_x-ang_bias_x;
		vmu_data.angular_velocity.y = ang_vel_y-ang_bias_y;
		vmu_data.angular_velocity.z = ang_vel_z-ang_bias_z;
	}
